...
const Login: React.FC<LoginProps> = ({ onClose, asModal, onSwitchToSignup }) => {
  ...
  return (
    <div className={asModal ? 'fixed inset-0 z-50 flex justify-center bg-background/80 mt-24 md:items-start items-start' : ''}>
      <div className={`flex w-full max-w-2xl mx-auto rounded-2xl shadow-2xl overflow-hidden ${asModal ? 'animate-scale-in' : 'my-12'}`} style={{background: 'hsl(var(--background))'}}>
        {/* Login Side */}
          ...
          {!showSignupForm ? (
            <>
              ...
              <div className="text-center mt-2">
                ...
                <button className="text-primary font-semibold hover:underline ml-1 flex items-center gap-1" onClick={() => setShowSignupForm(false)}>
